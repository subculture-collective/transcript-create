...
export default function VideoPage() {
  ...
  const { user } = useAuth()
  const [serverFavs, setServerFavs] = useState<Array<{ id: string; start_ms: number; end_ms: number }>>([])
  ...
  useEffect(() => {
    ...
    if (user) {
      apiListFavorites(videoId).then(r => {
        const filtered = r.items.map(i => ({ id: i.id, start_ms: i.start_ms, end_ms: i.end_ms }))
  ...
  async function toggleFavorite(id: number, seg: Segment) {
    ...
    if (user) {
      ...
      const existing = serverFavs.find(f => f.start_ms === seg.start_ms && f.end_ms === seg.end_ms)
      if (existing) {
        await apiDeleteFavorite(existing.id).catch(() => {})
        setServerFavs(prev => prev.filter(f => f.id !== existing.id))
        track({ type: 'favorite_remove', payload: { videoId, start_ms: seg.start_ms, end_ms: seg.end_ms } })
      } else {
        const res = await apiAddFavorite({ video_id: videoId, start_ms: seg.start_ms, end_ms: seg.end_ms, text: seg.text }).catch(() => null)
        if (res?.id) setServerFavs(prev => [{ id: res.id, start_ms: seg.start_ms, end_ms: seg.end_ms }, ...prev])
        track({ type: 'favorite_add', payload: { videoId, start_ms: seg.start_ms, end_ms: seg.end_ms } })
      }
    } else {
      favorites.toggle({ videoId, segIndex: id, startMs: seg.start_ms, endMs: seg.end_ms, text: seg.text })
    }
  }
  ...
  const matchIndices = useMemo(() => {
    ...
    transcript.segments.forEach((seg, idx) => {
      if (hits.find(h => h.start_ms >= seg.start_ms && h.start_ms < seg.end_ms)) ids.push(idx + 1)
    })
    return ids
  }, [transcript, hits])
  const [matchCursor, setMatchCursor] = useState(0)
  useEffect(() => { setMatchCursor(0) }, [matchIndices.join(',')])
          ...
          {video && (
            <>
              <button
                ...
                onClick={() => navigator.clipboard?.writeText(`${location.origin}/api/videos/${video.id}/youtube-transcript.srt`)}
              >Copy SRT URL</button>
              <button
                className="rounded border px-2 py-1 hover:bg-gray-50"
                onClick={() => navigator.clipboard?.writeText(`${location.origin}/api/videos/${video.id}/youtube-transcript.vtt`)}
              >Copy VTT URL</button>
            </>
          )}
        </div>
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          {matchIndices.length > 0 && (
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <button className="rounded border px-2 py-1 hover:bg-gray-50" onClick={() => gotoMatch(-1)}>Prev</button>
              <span>{matchCursor + 1} / {matchIndices.length}</span>
              <button className="rounded border px-2 py-1 hover:bg-gray-50" onClick={() => gotoMatch(1)}>Next</button>
            </div>
          )}
        </div>
        ...
        {transcript && (
          <ol className="space-y-0.5">
            {transcript.segments.map((seg, idx) => {
              ...
              const isServerFav = !!serverFavs.find(f => f.start_ms === seg.start_ms && f.end_ms === seg.end_ms)
              return (
                <li
                  id={`seg-${id}`}
                  key={id}
                ...
                >
                  <button onClick={() => onClickSegment(seg, id)} className="text-left">
                    <div className="mb-1 text-xs text-gray-500 font-mono">{msToHms(seg.start_ms)}</div>
                  ...
                  <div className="mt-1 hidden items-center gap-3 text-xs text-gray-600 group-hover:flex">
                    ...
                    <button className="hover:text-gray-900" onClick={() => toggleFavorite(id, seg)}>
                      {user ? (isServerFav ? 'Unsave' : 'Save') : (favorites.has({ videoId: videoId!, segIndex: id }) ? 'Unfavorite' : 'Favorite')}
                    </button>
                  </div>
                  {match && (
