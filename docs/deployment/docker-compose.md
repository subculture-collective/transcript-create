...
# Generate required interpolation secrets directly in .env.prod
printf '\nDB_PASSWORD=%s\n' "$(openssl rand -hex 32)" >> .env.prod
printf 'GRAFANA_ADMIN_PASSWORD=%s\n' "$(openssl rand -hex 32)" >> .env.prod
```

Production compose commands must pass `--env-file .env.prod`. Compose uses that
file for variable interpolation before it reads per-service `env_file` entries.
This production overlay uses the Docker Compose `!reset` tag to clear local-only
host port mappings; use a current Docker Compose V2 release that supports the
Compose specification reset tag.
...
# Run migrations
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml run --rm migrations
```
...
# Test GPU in worker
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec worker rocm-smi
# or for NVIDIA
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec worker nvidia-smi
...
```bash
# Production compose only publishes Caddy on 80/443.
# API, DB, Redis, OpenSearch, Prometheus, and Grafana stay on the compose network.
# Access monitoring through Caddy/SSH tunnel if needed.
...
```bash
# Database backup
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec backup \
...
# Restart services
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml start api worker
```
...
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d --no-deps --scale api=6 api
sleep 30
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d --no-deps --scale api=3 api

# 6. Update worker
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d --no-deps worker

# 7. Verify
curl https://api.example.com/health

# 8. Check logs for errors
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml logs --tail=100 api worker
```
...
# View logs
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml logs <service>

# Inspect container
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec <service> bash
```
...
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec caddy \
...
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec worker \
...
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec api \
...
docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d --scale worker=3
```
