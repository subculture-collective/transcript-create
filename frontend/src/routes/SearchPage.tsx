          ...
          {grouped.map(([videoId, group]) => (
            <div key={videoId} className="rounded-lg border p-4">
              <div className="mb-2 flex items-center justify-between">
                <Link to={`/v/${videoId}`} className="font-semibold hover:underline">Video {videoId.slice(0, 8)}…</Link>
              </div>
              <ul className="space-y-3">
                ...
                {group.length > 5 && (
                  <li>
                    <Link to={`/v/${videoId}?q=${encodeURIComponent(q)}`} className="text-blue-600 hover:underline">
