...
import { formatDate, formatDuration, formatVideoTitle } from '../../features/archive/format';
...
export default function VideoCard({ video }: VideoCardProps) {
  ...
  return (
    ...
    >
      <div className="line-clamp-2 font-semibold tracking-[-0.03em] text-ink">
        {formatVideoTitle(video.title, video.uploaded_at)}
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs">
        ...
        <span className="match-pill">
          {video.has_whisper_transcript ? 'whisper ready' : 'pending transcript'}
        </span>
      </div>
    </Link>
  );
