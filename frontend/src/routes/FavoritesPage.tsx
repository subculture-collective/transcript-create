...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    if (user) {
      apiListFavorites().then(r => setRemote(r.items)).catch(() => setRemote(null))
    } else {
      setRemote(null)
    }
  }, [user])
            ...
            {remote.map((f) => (
              <li key={f.id} className="flex items-start justify-between gap-4 rounded border p-3">
                <div>
                  ...
                  <Link className="text-blue-600 hover:underline" to={`/v/${f.video_id}?t=${Math.floor(f.start_ms/1000)}`}>Open</Link>
                </div>
                <button
